...
class TerrainGenerator:
    ...
    def generate_elevation(self, width, height, scale, octaves, persistence):
        """
    ...
    def generate_temperature(self, width, height, scale, equator_position):
        """
        ...
        for i in range(height):
            for j in range(width):
                base_temp = self.simplex.noise2(i / scale, j / scale)
                latitudinal_temp = 1 - abs(i - equator_position) / height
                temperature_map[i][j] = base_temp * latitudinal_temp
